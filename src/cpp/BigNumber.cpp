...
inline void NTT(Poly &a, int x, int type)
{
    ...
    if (type == -1)
        for (int i = 0, inv = quick_pow(n, mod - 2); i < n; ++i)
            a[i] = (long long)a[i] * inv % mod;
}
inline int ceilog(long long x)
{
    int ans = 0;
    while (1ll << ans < x)
        ans++;
...
BigNumber BigNumberAbsDiv(BigNumber num1, BigNumber num2, bool negative, bool isDiv)
{
    ...
    for (int s = 1; iteration_times > 0; iteration_times /= 2, s++)
    {
        ...
        for (int i = 0; i < std::min(1 << s, (int)num2.digits_.size()); ++i)
            C[i] = num2.digits_[i];
        for (int i = std::min(1 << s, (int)num2.digits_.size()); i < 1 << s; ++i)
            C[i] = 0;
        C = B * C;
