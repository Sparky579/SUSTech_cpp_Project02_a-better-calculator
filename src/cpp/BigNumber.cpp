...
BigNumber& BigNumber::checkCarry() {
	while (!digits_.empty() && !digits_.back()) digits_.pop_back();
	if (digits_.empty()) {
		dotpos_ = 0;
        negative_ = false;
		digits_.push_back(0);
...
std::string BigNumber::to_string() {
    std::string number = "";
    std::vector<int> effnum;
    int add_zero_num = dotpos_ - (int)digits_.size();
    for (int i = 1; i <= add_zero_num; i++) {
        effnum.push_back(0);
    }
    for (int i = digits_.size() - 1; i >= 0; i--) {
        effnum.push_back(digits_[i]);
    }
    int pos_dot = (int) effnum.size() - dotpos_;
    for (int i = 0; i < pos_dot; i++) {
        number += (char) (effnum[i] + '0');
    }
    if (dotpos_ != 0) number += '.';
    for (int i = pos_dot; i < effnum.size(); i++) {
        number += (char) (effnum[i] + '0');
    }
    if (number[0] == '.') number = "0" + number;
    if (negative_) number = "-" + number;
    return number;
}
void BigNumber::set_BigNumber (std::string number) {
    ...
    for (int i = 0; i < number.length(); i++) {
        if (number[i] == '.') pos_dot = i;
        if (number[i] == 'e') pos_e = i;
    }
    if (pos_e == -1) {
        dotpos_ = 0;
        for (int i = 0; i < number.length(); i++) {
            if (number[i] == '.') {
                dotpos_ = (int) number.length() - i - 1;
                break;
            }
        }
        ...
        int power = 0, power_sign = 1;
        if (number[pos_e + 1] == '-') {
            power_sign = -1;
        }
        for (int i = pos_e + 1; i < number.length(); i++) {
            if (number[i] >= '0' && number[i] <= '9') {
                power = power * 10 + (int) (number[i] - '0');
            }
        }
        power *= power_sign;
        if (power > 0) {
...
BigNumber& BigNumber::checkScale(int setted_scale) {
    std::reverse(digits_.begin(), digits_.end());
    for (int i = 1; i <= dotpos_ - setted_scale; i++) {
        digits_.pop_back();
        if (digits_.empty()) {
    ...
    for (int i = dotpos_ + 1; i <= setted_scale; i++) {
        digits_.push_back(0);
    }
...
BigNumber::BigNumber(int number_int) {
    set_BigNumber(std::to_string(number_int));
}
...
int BigNumberAbsCmp(BigNumber num1, BigNumber num2) {
    ...
    if (num1_is_zero && num2_is_zero) return 0;
    if (!num1_is_zero && num2_is_zero) return 1;
    if (num1_is_zero && !num2_is_zero) return -1;
    ...
    if (num1.dotpos_ < num2.dotpos_) {
        for (int i = 1; i <= num2.dotpos_ - num1.dotpos_; i++) {
            num1.digits_.push_back(0);
        }
    } else {
        for (int i = 1; i <= num1.dotpos_ - num2.dotpos_; i++) {
            num2.digits_.push_back(0);
        }
    }
    if (num1.digits_.size() > num2.digits_.size()) return 1;
    if (num1.digits_.size() < num2.digits_.size()) return -1;
    for (int i = 0; i < num1.digits_.size(); i++) {
        if (num1.digits_[i] > num2.digits_[i]) return 1;
        else if (num1.digits_[i] < num2.digits_[i]) return -1;
    }
    return 0;
}
...
BigNumber BigNumberAbsSub(BigNumber num1, BigNumber num2, bool negative) {
    ...
    std::reverse(num1.digits_.begin(), num1.digits_.end());
    std::reverse(num2.digits_.begin(), num2.digits_.end());
    for (int i = 0; i < num2.digits_.size(); num1.digits_[i] -= num2.digits_[i], i++)
		if (num1.digits_[i] < num2.digits_[i]) {
			int j = i + 1;
			while (!num1.digits_[j]) j++;
			while (j > i) {
				--num1.digits_[j];
				num1.digits_[--j] += 10;
			}
...
void FFT(std::vector<complex> &omega, int len, int no_reverse) {
	for (int i = 1, j = len / 2; i < len - 1; i++) {
		if (i < j) std::swap(omega[i], omega[j]);
		int k = len / 2;
		while (j >= k) {
			j -= k;
			k /= 2;
...
BigNumber BigNumberAbsMul(BigNumber num1, BigNumber num2, bool negative) {
	int len1 = num1.digits_.size(), len2 = num2.digits_.size(), len = 1;
	...
	FFT(complex_num1, len, 1); FFT(complex_num2, len, 1);
	for (int i = 0; i < len; i++) {
	...
	for (int i = 0; i < len; i++)
		sum.push_back((int)(complex_num1[i].Real + 0.5));
	for (int i = 0; i < len; i++) {
		sum[i + 1] += sum[i] / 10;
		sum[i] %= 10;
	}
	...
	std::vector<int>().swap(num1.digits_);
	for (int i = 0; i <= len; i++)
...
inline void NTT(Poly& a,int x,int type){
	int n=1ll<<x;
	std::vector<int>R; R.resize(n);
	for(int i=0;i<n;++i) if((R[i]=R[i>>1]>>1|(i&1)<<x-1)>i) std::swap(a[i],a[R[i]]);
	for(int s=1;s<=x;++s){
		int len=1ll<<s,mid=len>>1ll;
		int w=quick_pow(3ll,type*(mod - 1)/len+mod-1);
		for(int k=0;k<n;k+=len){
			int d=1;
			for(int j=0;j<mid;++j){
				int u=a[j+k],v=(long long)a[j+k+mid]*d%mod;
				a[j+k]=(u+v)%mod,a[j+k+mid]=(u-v+mod)%mod;
				d=(long long)d*w%mod;
			}
		}
	}
	if(type==-1) for(int i=0,inv=quick_pow(n,mod-2);i<n;++i) a[i]=(long long)a[i]*inv%mod;
}
inline int ceilog(long long x){int ans=0;while(1ll<<ans<x)ans++;return ans;}
Poly operator * (Poly A,Poly B){
	int deg=A.size()+B.size()-1;
	int x=ceilog(deg);
	A.resize(1<<x),B.resize(1<<x);
	NTT(A,x,1),NTT(B,x,1);
	for(int i=0;i<1<<x;++i) A[i]=(long long)A[i]*B[i]%mod;
	NTT(A,x,-1);
	A.resize(deg);
	for(int i=deg-1;i;--i){
		A[i-1]+=A[i]/10;
...
BigNumber BigNumberAbsDiv(BigNumber num1, BigNumber num2, bool negative, bool isDiv) {
    ...
    int limits = (int) num1.digits_.size() - (int) num2.digits_.size() + scale + 16;
    ...
    num1.dotpos_ = num2.dotpos_ = std::max(num1.dotpos_, num2.dotpos_);
	int rest = num1.digits_.size() - num2.digits_.size() + (isDiv ? 0 : scale);
	Poly A,B,C;B.resize(2);B[1]=100/(num2.digits_[0]*10+(num2.digits_.size()>1?num2.digits_[1]:0));
	int x=ceilog(num2.digits_.size())+1;
    for(int s = 1; iteration_times > 0; iteration_times /= 2, s++){
		C.resize(1<<s),B.resize(1<<s);
		for(int i=0;i<std::min(1<<s,(int)num2.digits_.size());++i) C[i]=num2.digits_[i];
		for(int i=std::min(1<<s,(int)num2.digits_.size());i<1<<s;++i) C[i]=0;
		C=B*C;
...
BigNumber trunc(BigNumber num) {
    ...
    int digit_size = num.digits_.size();
    for (int i = 1; i <= num.dotpos_ && i <= digit_size; i++) {
        num.digits_.pop_back();
    }
    num.dotpos_ = 0;
    std::reverse(num.digits_.begin(), num.digits_.end());
    if (num.digits_.empty()) num = 0;
    return num;
}
BigNumber floor(BigNumber num) {
    BigNumber trunc_num = trunc(num);
    if (trunc_num == num) return num;
    if (num < 0) return trunc_num - 1;
    else return trunc_num;
}
BigNumber ceil(BigNumber num) {
    BigNumber trunc_num = trunc(num);
    if (trunc_num == num) return num;
    if (num < 0) return trunc_num;
    else return trunc_num + 1;
}
...
BigNumber sqrt(BigNumber num) {
    ...
    while (true) {
        ...
        if (BigNumberAbsCmp(cur_num - last_num, eps) <= 0) break;
        last_num = cur_num;
    }
    if (last_num == cur_num) last_num = cur_num + eps;
    if (last_num < cur_num) std::swap(cur_num, last_num);
    if (last_num * last_num <= num) return last_num;
    else return cur_num;
}
...
BigNumber exp(BigNumber num) {
    if (num == 0) return 1;
    if (num < 0) return (1 / exp(0 - num)).checkScale(scale);
    int count10 = num.digits_.size() - num.dotpos_;
    num.dotpos_ = num.digits_.size();
    std::string power = "1";
    for (int i = 1; i <= count10; i++) power += '0';
    BigNumber eps;
    eps.negative_ = false;
    eps.digits_.push_back(1);
    eps.dotpos_ = scale;
    ...
    for (int i = 1; ; i++) {
        ...
        if (BigNumberAbsCmp(now_item, eps) <= 0) break;
        cur_exp += now_item;
    }
...
BigNumber small_ln(BigNumber num) {
    ...
    for (int i = 3; ; i += 2) {
        ...
        if (BigNumberAbsCmp(add_item, eps) <= 0) break;
...
BigNumber ln(BigNumber num) {
    ...
    while (true) {
        num = sqrt(num);
        power += 1;
        if (BigNumberAbsCmp(num - 1, eps) <= 0) break;
    }
...
BigNumber arctan(BigNumber num) {
    if (num.negative_) return 0 - arctan(0 - num);
    if (num > 1) return get_2pi() - arctan(1 / num);
    if (num > 0.5) {
...
BigNumber operator ^ (BigNumber num1, BigNumber num2) {
    ...
    if (trunc(num2) == num2) {
        int setted_scale = std::max(scale, num1.dotpos_);
        num2 = trunc(num2);
        BigNumber ans = 1;
        while (num2 != 0) {
            if (num2.digits_[0] % 2 == 1) ans = (ans * num1).checkScale(setted_scale);
    ...
    for (int i = 1; ; i++) {
        ...
        if (BigNumberAbsCmp(cur_item, eps) <= 0) break;
        cur_proid += cur_item;
    }
    if (reciprocal) {
